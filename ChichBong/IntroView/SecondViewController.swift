...
class SecondViewController: UIViewController, UITextFieldDelegate {
    ...
    func showOptionError(mes:String) {
        let alert = UIAlertController(title: "Chú ý", message: mes, preferredStyle: .alert)
        
        let actionDe = UIAlertAction.init(title: "Thử lại", style: .default) { (action) in
            //
            alert.dismiss(animated: true, completion: nil)
            return
        }
        
        let actionCancle = UIAlertAction.init(title: "Bỏ qua", style: .cancel) { (action) in
            NotificationCenter.default.post(name:.goIntoApp, object: nil);
        }
